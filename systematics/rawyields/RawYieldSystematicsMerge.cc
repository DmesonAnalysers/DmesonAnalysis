...
int RawYieldSystematicsMerge() {
    ...
    TNtuple* ntuple2Fit[nPtBins];
    TNtuple* ntuple2BinC[nPtBins];
    TFile a("/home/fchinu/Xic0_pPb_5TeV/Dmesoncode/Systematics/RawYieldSyst_2_4_Pol2_Expo_Fixed_Xic0.root","read");
    ntuple2Fit[0]=(TNtuple *)a.Get("ntupleFit_pT_20-40");
    ntuple2Fit[0]->SetDirectory(0);
    ntuple2BinC[0]=(TNtuple *)a.Get("ntupleBinC_pT_20-40");
    ntuple2BinC[0]->SetDirectory(0);
    a.Close();
    ...
    for (int iPt = 0; iPt < nPtBins; iPt++) {
        ...
        TNtuple* ntupleFittest[1];
        TNtuple* ntupleBinCtest[1];
        ntupleFittest[iPt] = (TNtuple *)(multiTrial.GetNtupleMultiTrials())->Clone(Form("ntupleFit_pT_%0.f-%0.f", PtLims[iPt]*10, PtLims[iPt+1]*10));
        ntupleBinCtest[iPt] = (TNtuple *)(multiTrial.GetNtupleBinCounting())->Clone(Form("ntupleBinC_pT_%0.f-%0.f", PtLims[iPt]*10, PtLims[iPt+1]*10));
        ntupleFittest[iPt]->SetDirectory(0);
        ntupleBinCtest[iPt]->SetDirectory(0);

        
        TList *list1 = new TList;
        list1->Add(ntuple2Fit[0]);
        list1->Add(ntupleFittest[0]);
        ntupleFit[iPt]=(TNtuple *)TTree::MergeTrees(list1);

        TList *list2 = new TList;
        list2->Add(ntuple2BinC[0]);
        list2->Add(ntupleBinCtest[0]);
        ntupleBinC[iPt]=(TNtuple *)TTree::MergeTrees(list2);
