...
for iFile, (inFileName, objName, objType, scale, color, marker) in \
    ...
    SetObjectStyle(hToCompare[iFile], color=GetROOTColor(color), markerstyle=GetROOTMarker(marker), markersize=inputCfg['options']['markersize'],
                   linewidth=inputCfg['options']['linewidth'], fillstyle=inputCfg['options']['fillstyle'][iFile])
    if 'TH' in objType:
