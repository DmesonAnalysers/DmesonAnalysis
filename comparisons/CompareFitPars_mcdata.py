...
def comp_fit_pars(do_ratio=False, meson='Ds'): #pylint: disable-msg=too-many-statements,too-many-locals
    inputdir = '/home/fchinu/Xic0_pPb_5TeV/Dmesoncode/wSDD_newtrains'
    input_files = ['RawYieldsData.root']
    input_files_MC = ['RawYieldsMC.root']
    colors = [kAzure+3, kRed, kOrange-3, kAzure+3, kBlack, kGreen-2, kOrange-3]
    markers = [kOpenCircle, kOpenSquare, kFullCross, kFullCircle,kFullDiamond,kFullSquare, kOpenCross]
    legendnames = ['MC', 'data']
    suffix = 'FitParsComparison'
    min_pt = 2.
    max_pt = 12.
    ...
    if meson == 'Ds':
        ...
        massD = TDatabasePDG.Instance().GetParticle(4132).Mass()
