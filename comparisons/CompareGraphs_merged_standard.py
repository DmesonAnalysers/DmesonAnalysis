...
def StdMerge(hCons, hStrong, ptSwitch = 6):
    ...
    #hMerged.SetBinContent(1,-999)
    return  hMerged

temp = [] 

mergeFileNames = ['/home/fchinu/Ds_pp_13TeV/output_analysis/stefano/HFPtSpectrum_CentralConsPID.root',
                    '/home/fchinu/Ds_pp_13TeV/output_analysis/stefano/HFPtSpectrum_CentralStrongPID.root']

for iFile, inFileName in enumerate(mergeFileNames):
    inFile = TFile.Open(inFileName)
    temp.append(inFile.Get('histoSigmaCorr'))
    temp[iFile].SetDirectory(0)
    inFile.Close()

file = TFile.Open('/home/fchinu/Ds_pp_13TeV/output_analysis/stefano/mergedsigma.root','recreate')
StdMerge(temp[0],temp[1]).Write('histoSigmaCorr')
file.Close()

inFileNames.pop()
inFileNames.pop()
inFileNames.append('/home/fchinu/Ds_pp_13TeV/output_analysis/stefano/mergedsigma.root')
...
if 'TH' in objTypes[2]:
        hToCompare[2].DrawCopy(f'{drawOptions[2]}same')
else:
    hToCompare[2].Draw(drawOptions[2])

for iFile, (histo, objType, drawOpt) in enumerate(zip(hToCompare, objTypes, drawOptions)):
    if iFile==2:
        continue
    if 'TH' in objType:
