...
def comp_fit_pars(do_ratio=False, meson='Ds'): #pylint: disable-msg=too-many-statements,too-many-locals
    ...
    input_files_MC = ['/final/RawYieldDs_MC_pp13TeV.root']
    mergeFileNames = ['/stefano/RawYieldsDs_CentralConsPID.root','/stefano/RawYieldsDs_CentralStrongPID.root']
    colors = [kAzure+3, kAzure+3, kGreen-2, kOrange-3]
    markers = [kOpenCircle, kFullCircle,kFullDiamond,kFullSquare, kOpenCross]
    legendnames = ['MC', 'data - binary' ,'data - multiclass', 'data - standard ']
    ...
    for iFile, inFileName in enumerate(mergeFileNames):
        ...
        tempMean.append(inFile.Get('hRawYieldsMean'))
        tempSigma.append(inFile.Get('hRawYieldsSigma'))
        tempMean[iFile].SetDirectory(0)
        tempSigma[iFile].SetDirectory(0)
        inFile.Close()

    file = TFile.Open('/home/fchinu/Ds_pp_13TeV/output_analysis/stefano/mergedfitpars.root','recreate')
    StdMerge(tempMean[0],tempMean[1]).Write('hRawYieldsMean')
    StdMerge(tempSigma[0],tempSigma[1]).Write('hRawYieldsSigma')
    file.Close()

    input_files.pop()
    input_files.pop()
    input_files.append('/stefano/mergedfitpars.root')
    ...
    cMean.SaveAs(f'{inputdir}/final/thesis/Mean_{suffix}.pdf')
    cSigma.SaveAs(f'{inputdir}/final/thesis/Sigma_{suffix}.pdf')
    cMean.SaveAs(f'{inputdir}/final/thesis/Mean_{suffix}.png')
    cSigma.SaveAs(f'{inputdir}/final/thesis/Sigma_{suffix}.png')
