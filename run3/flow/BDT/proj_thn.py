    ...
    with alive_bar(len(cutVars['Pt']['min']), title='Processing pT bins') as bar:
        for iPt, (ptMin, ptMax) in enumerate(zip(cutVars['Pt']['min'], cutVars['Pt']['max'])):
            ...
            if args.proj_mc:
                ...
                histo_pt_gen_prompt = outfile_old.Get(f'cent_bins{cent}/pt_bins{ptMin}_{ptMax}/hPromptGenPt').Clone()
                histo_pt_gen_fd = outfile_old.Get(f'cent_bins{cent}/pt_bins{ptMin}_{ptMax}/hFDGenPt').Clone()
                histo_pt_reco_prompt = outfile_old.Get(f'cent_bins{cent}/pt_bins{ptMin}_{ptMax}/hPromptPt').Clone()
                histo_pt_reco_fd = outfile_old.Get(f'cent_bins{cent}/pt_bins{ptMin}_{ptMax}/hFDPt').Clone()
                histo_prompt_mass = outfile_old.Get(f'cent_bins{cent}/pt_bins{ptMin}_{ptMax}/hPromptMass').Clone()
                histo_fd_mass = outfile_old.Get(f'cent_bins{cent}/pt_bins{ptMin}_{ptMax}/hFDMass').Clone()
                outfile.cd(f'cent_bins{cent}/pt_bins{ptMin}_{ptMax}')
                histo_prompt_mass.Write('hPromptMass')
                histo_pt_gen_prompt.Write('hPromptGenPt')
                histo_pt_gen_fd.Write('hFDGenPt')
                histo_pt_reco_fd.Write('hFDPt')
                histo_pt_reco_prompt.Write('hPromptPt')
                histo_fd_mass.Write('hFDMass')
                print(f"Loaded mc from previous projections!")
