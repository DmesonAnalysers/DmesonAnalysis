...
def combination_fit_option(config_flow_name, cfg_flow, nPtBins, cfg_mod):
    ...
    flow_configs_original_mass_bins = {} # key: config name, value: config dict
    flow_configs[config_flow_name] = copy.deepcopy(cfg_flow)
    flow_configs_original_mass_bins[config_flow_name + '-ori'] = copy.deepcopy(cfg_flow)

    # sigma
    if fit_option_dict['Sigma']['FixSigma'] == 0:
        terms_FixSigma = [[0 for _ in range(nPtBins)] for _ in range(2)]
        ...
        sigma_file.Close()
        terms_FixSigma = [1 for _ in range(2)]
        terms_FixSigmaFromFile = ['' for  _ in range(2)]
        if fit_option_dict['Sigma']['FixSigma'] == -1:
            ...
            terms_sigma = [Sigma_lowers, Sigma_uppers]
            ...
            flow_configs_original_mass_bins = generate_flow_config_variations_add(flow_configs_original_mass_bins, multi_terms=[terms_FixSigma, terms_sigma, terms_FixSigmaFromFile], multi_terms_name=['FixSigma', 'Sigma', 'FixSigmaFromFile'])
        else:
    ...
    flow_configs_original_mass_bins.pop(config_flow_name + '-ori', None)
    ...
    flow_configs_original_mass_bins = generate_flow_config_variations_add(flow_configs_original_mass_bins, multi_terms=[terms_bkg_func_vn], multi_terms_name=['BkgFuncVn'])
    ...
    flow_configs_original_mass_bins = generate_flow_config_variations_add(flow_configs_original_mass_bins, multi_terms=[terms_bkg_func], multi_terms_name=['BkgFunc'])
    ...
    flow_configs_original_mass_bins = generate_flow_config_variations_add(flow_configs_original_mass_bins, multi_terms=[terms_rebin], multi_terms_name=['Rebin'])
    ...
    # flow_configs_original_mass_bins = generate_flow_config_variations_add(flow_configs_original_mass_bins, multi_terms=[terms_use_inv_mass_bins], multi_terms_name=['use_inv_mass_bins'])

    flow_configs.update(flow_configs_original_mass_bins)
    return flow_configs, fit_opts_dependent_pt

def slice_single_pt(flow_configs, nPtBins, fit_opts_dependent_pt):
...
def produce_pre_config(cfg_flow, cfg_mod, output_dir):
    ...
    pre_config_dict = {}
    pre_config_dict['flow_files'] = cfg_mod['flow_files']
    pre_config_dict['ptmins'] = cfg_flow['ptmins']
    pre_config_dict['ptmaxs'] = cfg_flow['ptmaxs']
    pre_config_dict['centrality'] = cfg_mod['centrality']
    ...
    pre_config_dict['axestokeep'] = ['Mass', 'sp', 'score_FD', 'score_bkg']
...
def modify_yaml_bdt(config_flow, config_mod, output_dir):
    ...
    #! modify it if you have other pre-processed files
    cfg_flow['skim_out_dir'] = f'{output_dir}/pre/AnRes'
    ...
    flow_configs, fit_opts_dependent_pt = combination_fit_option(config_flow, cfg_flow, nPtBins, cfg_mod)
    ...
    flow_configs_pt = slice_single_pt(flow_configs, nPtBins, fit_opts_dependent_pt)
    ...
    # default config uncorrelated with config_pre
    with open(f'{output_dir}/config_sys/config_default.yml', 'w') as f:
        cfg_default = copy.deepcopy(cfg_flow)
        cfg_default['skim_out_dir'] = f'{output_dir}/pre/AnRes'
        yaml.dump(cfg_default, f, default_flow_style=False)
        
    with open(f'{output_dir}/config_sys/config_default_corelated.yml', 'w') as f:
        cfg_default_correlated = copy.deepcopy(cfg_default)
        cfg_default_correlated['minimisation']['correlated'] = True
        yaml.dump(cfg_default_correlated, f, default_flow_style=False)
