...
inputfilenames = ['HFPtSpectrumRaaDs_010_RightPredFile_2015.root','HFPtSpectrumRaaDs_010_central_2015.root','HFPtSpectrumRaaDs_010_central_2015_oldppref.root']
histonames = ['hRABvsPt','hRABvsPt','hRABvsPt']
graphnames = ['gRAB_GlobalSystematics','gRAB_GlobalSystematics','gRAB_GlobalSystematics']
colors = [ kRed, kBlue, kBlack, kBlue, kRed ]
linecolors = [ kRed, kBlue, kBlack, kBlue, kRed ]
markers = [ kFullSquare, kFullCircle, kOpenCircle ]
legendnames = [ 'D_{s}^{+} 2015 (2015 cuts)', 'D_{s}^{+} 2018 (2015 cuts)', 'D_{s}^{+} 2018 (2015 cuts, 2015 pp ref)' ]
...
ptmax = hRaa[0].GetBinLowEdge(hRaa[0].GetNbinsX())+hRaa[0].GetBinWidth(hRaa[0].GetNbinsX())
...
cRaa.Divide(2,1)
cRaa.cd(1).DrawFrame(ptmin,0.,ptmax,2.,';#it{p}_{T} (GeV/#it{c}); #it{R}_{AA}')
#cRaa.SetLogx()
lineatone.Draw('same')
for iFile in range(len(inputfilenames)):
  # gRaa[iFile].Draw('2')
...
leg.Draw()
cRaa.cd(2).DrawFrame(ptmin,0.,ptmax,2.,';#it{p}_{T} (GeV/#it{c}); ratio #it{R}_{AA}')
for iFile in range(len(inputfilenames)):
  if iFile==0:
    continue
  # gRaa[iFile].Draw('2')
