...
def ComputeExpectedFprompt(ptmin, ptmax, effPrompt, hPredPrompt, hPredPromptMin, hPredPromptMax, effFD, hPredFD, hPredFDMin, hPredFDMax, RatioRaaFDPrompt):
    ...
    predPrompt, predPromptMin, predPromptMax, predFD, predFDMin, predFDMax = (0 for iPred in range(6))
    for iPt in range(ptbinmin, ptbinmax+1):
        predPrompt += hPredPrompt.GetBinContent(iPt) * hPredPrompt.GetBinWidth(iPt)
        predPromptMin += hPredPromptMin.GetBinContent(iPt) * hPredPromptMin.GetBinWidth(iPt)
        predPromptMax += hPredPromptMax.GetBinContent(iPt) * hPredPromptMax.GetBinWidth(iPt)
        predFD += hPredFD.GetBinContent(iPt) * hPredFD.GetBinWidth(iPt)
        predFDMin += hPredFDMin.GetBinContent(iPt) * hPredFDMin.GetBinWidth(iPt)
        predFDMax += hPredFDMax.GetBinContent(iPt) * hPredFDMax.GetBinWidth(iPt)
...
colorsFill = {'TAMU': kOrange+7, 'PHSD': kGreen+2, 'Gossiaux': kAzure+2, 'Catania': kRed}
...
if inputCfg['PredForFprompt']['estimateFprompt']:
    ...
    hPredPromptMin = infilePred.Get(inputCfg['PredForFprompt']['histonamePrompt'].replace('central','min'))
    hPredPromptMax = infilePred.Get(inputCfg['PredForFprompt']['histonamePrompt'].replace('central','max'))
    hPredFD = infilePred.Get(inputCfg['PredForFprompt']['histonameFD'])
    hPredFDMin = infilePred.Get(inputCfg['PredForFprompt']['histonameFD'].replace('central','min'))
    hPredFDMax = infilePred.Get(inputCfg['PredForFprompt']['histonameFD'].replace('central','max'))
...
hSignifTAMU, hSignifPHSD, hSignifGossiaux, hSignifCatania = (TH1F('hSignificance%s' % names[iGraph], ';#it{p}_{T} (GeV/#it{c});significance', len(cutVars['Pt']['limits'])-1, array.array('f', cutVars['Pt']['limits'])) for iGraph in range(4))
gCorrYieldTAMU, gCorrYieldPHSD, gCorrYieldGossiaux, gCorrYieldCatania = (TGraphAsymmErrors(0) for iGraph in range(4))
gRawYieldTAMU, gRawYieldPHSD, gRawYieldGossiaux, gRawYieldCatania = (TGraphAsymmErrors(0) for iGraph in range(4))
hRawYieldTAMU, hRawYieldPHSD, hRawYieldGossiaux, hRawYieldCatania = (TH1F('hRawYield%s' % names[iGraph], ';#it{p}_{T} (GeV/#it{c});raw yield', len(cutVars['Pt']['limits'])-1, array.array('f', cutVars['Pt']['limits'])) for iGraph in range(4))
...
hBackground = TH1F('hBackground', ';#it{p}_{T} (GeV/#it{c});B(3#sigma)', len(cutVars['Pt']['limits'])-1, array.array('f', cutVars['Pt']['limits']))
...
for iPt, ptmin in enumerate(cutVars['Pt']['min']):
    ...
    if inputCfg['PredForFprompt']['estimateFprompt']:
        fprompt, fpromptmin, fpromptmax = ComputeExpectedFprompt(ptmin, ptmax, effPrompt, hPredPrompt, hPredPromptMin, hPredPromptMax, effFD, hPredFD, hPredFDMin, hPredFDMax, RatioRaaFDPrompt)
    else:
        fprompt, fpromptmin, fpromptmax = (inputCfg['fprompt'] for iF in range(3))        

    gFprompt.SetPoint(iPt, PtCent, fprompt)
    gFprompt.SetPointError(iPt, PtUnc, PtUnc, fprompt-fpromptmin, fpromptmax-fprompt)

    SignifTAMU, SignifPHSD, SignifGossiaux, SignifCatania = ({} for iDic in range(4))
    SignifUncTAMU, SignifUncPHSD, SignifUncGossiaux, SignifUncCatania = ({} for iDic in range(4))
    CorrYieldTAMU, CorrYieldPHSD, CorrYieldGossiaux, CorrYieldCatania = ({} for iDic in range(4))
    RawYieldTAMU, RawYieldPHSD, RawYieldGossiaux, RawYieldCatania = ({} for iDic in range(4))
    for iFONLL in FONLL:
        if iFONLL == 'Cent' or iFONLL == 'Min' or iFONLL == 'Max':

            SignifTAMU[iFONLL], SignifUncTAMU[iFONLL], CorrYieldTAMU[iFONLL], RawYieldTAMU[iFONLL] = (dict(RaaTAMU) for iDic in range(4)) 
            SignifPHSD[iFONLL], SignifUncPHSD[iFONLL], CorrYieldPHSD[iFONLL], RawYieldPHSD[iFONLL] = (dict(RaaPHSD) for iDic in range(4)) 
            SignifGossiaux[iFONLL], SignifUncGossiaux[iFONLL], CorrYieldGossiaux[iFONLL], RawYieldGossiaux[iFONLL] = (dict(RaaGossiaux) for iDic in range(4)) 
            SignifCatania[iFONLL], SignifUncCatania[iFONLL], CorrYieldCatania[iFONLL], RawYieldCatania[iFONLL] = (dict(RaaCatania) for iDic in range(4)) 
            ...
            for iRaaTAMU in RaaTAMU:
                ...
                SignifUncTAMU[iFONLL][iRaaTAMU] = rawyield/math.sqrt(rawyield+B)
            ...
            for iRaaPHSD in RaaPHSD:
                ...
                SignifUncPHSD[iFONLL][iRaaPHSD] = rawyield/math.sqrt(rawyield+B)
            ...
            for iRaaGossiaux in RaaGossiaux:
                ...
                SignifUncGossiaux[iFONLL][iRaaGossiaux] = rawyield/math.sqrt(rawyield+B)
            ...
            for iRaaCatania in RaaCatania:
                ...
                SignifUncCatania[iFONLL][iRaaCatania] = rawyield/math.sqrt(rawyield+B)
    ...
    gRawYieldTAMU.SetPointError(iPt, PtUnc, PtUnc, RawYieldTAMU['Max']['Min']-RawYieldTAMU['Min'][min(RawYieldTAMU['Min'], key=RawYieldTAMU['Min'].get)], RawYieldTAMU['Max'][max(RawYieldTAMU['Max'], key=RawYieldTAMU['Max'].get)]-RawYieldTAMU['Max']['Min'])
    gRawYieldPHSD.SetPoint(iPt, PtCent, RawYieldPHSD['Max']['Cent'])
    gRawYieldPHSD.SetPointError(iPt, PtUnc, PtUnc, RawYieldPHSD['Max']['Cent']-RawYieldPHSD['Min'][min(RawYieldPHSD['Min'], key=RawYieldPHSD['Min'].get)], RawYieldPHSD['Max'][max(RawYieldPHSD['Max'], key=RawYieldPHSD['Max'].get)]-RawYieldPHSD['Max']['Cent'])
    gRawYieldGossiaux.SetPoint(iPt, PtCent, RawYieldGossiaux['Max']['ColRad'])
    gRawYieldGossiaux.SetPointError(iPt, PtUnc, PtUnc, RawYieldGossiaux['Max']['ColRad']-RawYieldGossiaux['Min'][min(RawYieldGossiaux['Min'], key=RawYieldGossiaux['Min'].get)], RawYieldGossiaux['Max'][max(RawYieldGossiaux['Max'], key=RawYieldGossiaux['Max'].get)]-RawYieldGossiaux['Max']['ColRad'])
    gRawYieldCatania.SetPoint(iPt, PtCent, RawYieldCatania['Max']['Cent'])
    gRawYieldCatania.SetPointError(iPt, PtUnc, PtUnc, RawYieldCatania['Max']['Cent']-RawYieldCatania['Min'][min(RawYieldCatania['Min'], key=RawYieldCatania['Min'].get)], RawYieldCatania['Max'][max(RawYieldCatania['Max'], key=RawYieldCatania['Max'].get)]-RawYieldCatania['Max']['Cent'])
    ...
    hSignifTAMU.SetBinError(iPt+1,math.sqrt(SignifTAMU['Max']['Min'])*math.sqrt((siguncTAMU**2+Bunc**2)/(4.*(sigTAMU+B)**2)+(B/(sigTAMU+B))*(siguncTAMU/sigTAMU)**2))
    hSignifPHSD.SetBinContent(iPt+1,SignifPHSD['Max']['Cent'])
    hSignifPHSD.SetBinError(iPt+1,math.sqrt(SignifPHSD['Max']['Cent'])*math.sqrt((siguncPHSD**2+Bunc**2)/(4.*(sigPHSD+B)**2)+(B/(sigPHSD+B))*(siguncPHSD/sigPHSD)**2))
    hSignifGossiaux.SetBinContent(iPt+1,SignifGossiaux['Max']['ColRad'])
    hSignifGossiaux.SetBinError(iPt+1,math.sqrt(SignifGossiaux['Max']['ColRad'])*math.sqrt((siguncGossiaux**2+Bunc**2)/(4.*(sigGossiaux+B)**2)+(B/(sigGossiaux+B))*(siguncGossiaux/sigGossiaux)**2))
    hSignifCatania.SetBinContent(iPt+1,SignifCatania['Max']['Cent'])
    hSignifCatania.SetBinError(iPt+1,math.sqrt(SignifCatania['Max']['Cent'])*math.sqrt((siguncCatania**2+Bunc**2)/(4.*(sigCatania+B)**2)+(B/(sigCatania+B))*(siguncCatania/sigCatania)**2))
...
legGossiaux['Signif'].AddEntry(gCorrYieldGossiaux, 'FONLL x MC@sHQ+EPOS2', 'fp')
...
legCatania['Signif'].AddEntry(gCorrYieldCatania, 'FONLL x Catania (frag+coal)', 'fp')
...
SetGraphStyle(gCorrYieldGossiaux, colors['Gossiaux'], kWhite, markers['Gossiaux'])
...
cRawYield.cd(1).DrawFrame(cutVars['Pt']['min'][0]-1, hRawYieldTAMU.GetMinimum()*0.5, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, hRawYieldTAMU.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});raw yield')
...
cRawYield.cd(2).DrawFrame(cutVars['Pt']['min'][0]-1, hRawYieldPHSD.GetMinimum()*0.5, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, hRawYieldPHSD.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});raw yield')
...
cRawYield.cd(3).DrawFrame(cutVars['Pt']['min'][0]-1, hRawYieldGossiaux.GetMinimum()*0.5, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, hRawYieldGossiaux.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});raw yield')
SetGraphStyle(gRawYieldGossiaux, colors['Gossiaux'], kWhite, markers['Gossiaux'])
SetGraphStyle(hRawYieldGossiaux, colors['Gossiaux'], kWhite, markers['Gossiaux'])
...
cRawYield.cd(4).DrawFrame(cutVars['Pt']['min'][0]-1, hRawYieldCatania.GetMinimum()*0.5, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, hRawYieldCatania.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});raw yield')
...
cSignif.Divide(2, 2)
cSignif.cd(1).DrawFrame(cutVars['Pt']['min'][0]-1, 0, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, hSignifTAMU.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});significance')
...
cSignif.cd(2).DrawFrame(cutVars['Pt']['min'][0]-1, 0, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, hSignifPHSD.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});significance')
...
cSignif.cd(3).DrawFrame(cutVars['Pt']['min'][0]-1, 0, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, gSignifGossiaux.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});significance')
...
cSignif.cd(4).DrawFrame(cutVars['Pt']['min'][0]-1, 0, cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1]+1, hSignifCatania.GetMaximum()*1.5, ';#it{p}_{T} (GeV/#it{c});significance')
...
cFprompt.DrawFrame(cutVars['Pt']['min'][0], 0., cutVars['Pt']['max'][len(cutVars['Pt']['max'])-1], 1., ';#it{p}_{T} (GeV/#it{c});#it{f}_{prompt} (#it{f}_{c})')
gFprompt.Draw('p')
