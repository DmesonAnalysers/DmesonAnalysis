...
void MakeFileForCuts_Central2015(Bool_t fUseStrongPID = kTRUE, Double_t maxPtstrongPID = 8.0) {
    ...
    Float_t topomCuts[nptbins] = {1.5,1.5,2.5,2.5,2.5,2.5}; //topomatic
    ...
    TFile* fout=new TFile(Form("DstoKKpiCuts_3050_central_strongPIDpt%0.f_Raa_kINT7_kSemiCentral.root",maxPtstrongPID),"recreate");
    fout->cd();
    analysiscuts->Write();
    fout->Close();
}
...
void MakeFileForCuts_Central2018(Bool_t fUseStrongPID = kTRUE, Double_t maxPtstrongPID = 8.0) {
    ...
    anacutsval[6][0]=0.035;   //sigmavert
    anacutsval[7][0]=0.04;   //decay length
    anacutsval[9][0]=0.985;   //cosP
    anacutsval[12][0]=0.005; //Mass Phi
    anacutsval[14][0]=0.2;  //Abs(CosineKpiPhiRFrame)^3
    anacutsval[15][0]=0.9;  //CosP labFrame
    ...
    anacutsval[12][1]=0.005; //Mass Phi
    anacutsval[14][1]=0.15;  //Abs(CosineKpiPhiRFrame)^3
    anacutsval[15][1]=0.9;  //CosP labFrame
    ...
    anacutsval[15][2]=0.9;   //CosP labFrame
    ...
    anacutsval[15][4]=0.95;  //CosP labFrame
    ...
    anacutsval[9][5]=0.975;   //cosP
    anacutsval[12][5]=0.005; //Mass Phi
    anacutsval[14][5]=0.;   //Abs(CosineKpiPhiRFrame)^3
    anacutsval[15][5]=0.95;  //CosP labFrame
    ...
    anacutsval[18][6]=5.0;   //normdecayXY
    ...
    anacutsval[19][7]=0.975;  //CosPXY

    anacutsval[6][8]=0.040;   //sigmavert
    anacutsval[7][8]=0.05;   //decay length
    anacutsval[9][8]=0.975;   //cosP
    ...
    anacutsval[18][8]=5.0;   //normdecayXY
...
void MakeFileForCuts_Loose2018(Bool_t fUseStrongPID = kTRUE, Double_t maxPtstrongPID = 8.0) {
    ...
    Float_t topomCuts[nptbins] = {3.,4.,4.,5.,5.,5.,6.,6.,6.}; //topomatic
    Float_t d0Cuts[nptbins] = {300.,300.,300.,300.,300.,300.,300.,300.,300.}; //impparXY
    ...
    analysiscuts->PrintAll();
    TFile* fout=new TFile(Form("DstoKKpiCuts_3050_loose_strongPIDpt%0.f_Raa_kINT7_kSemiCentral.root",maxPtstrongPID),"recreate");
    fout->cd();
    analysiscuts->Write();
    fout->Close();
}