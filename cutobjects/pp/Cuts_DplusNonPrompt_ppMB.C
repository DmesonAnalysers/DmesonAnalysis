...
// 2) MakeFileForCutsDpluspp5TeV_LowPtTreeML --> loose cuts for 2029 analysis (low pT prompt)
//____________________________________________________________________________________________________//
...
//__________________________________________________________________________________________
AliRDHFCutsDplustoKpipi *MakeFileForCutsDpluspp5TeV_LowPtTreeML(bool fUseStrongPID = false, double maxPtstrongPID = -1.0, bool fIsMC = false)
{
    ...
    //pT 0-1
    anacutsval[6][0] = 0.050; //sigvert
    anacutsval[9][0] = 0.7; //cospxy
    anacutsval[13][0] = 0.7; //cospxy
    ...
    analysiscuts->PrintAll();
    analysiscuts->PrintTrigger();
    TString filename = Form("DplustoKpipiCuts_pp_prompt_pt_0-1_loose%s_%s.root", PIDsuffix.Data(), triggername.Data());
    TFile *fout = new TFile(filename.Data(), "recreate");
    fout->cd();
    analysiscuts->Write();
    fout->Close();
