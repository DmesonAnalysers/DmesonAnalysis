...
for iPt in range(PtMin.len()):
    ...
    print('Applying selections')
    dataFramePtCut = dataFrame.query(f'pt_cand > {PtMin[iPt]} & pt_cand < {PtMax[iPt]}')
    del dataFrame
    if preSelections:
        dataFramePtCutSel = dataFramePtCut.astype(float).query(preSelections)
        del dataFramePtCut
            ...
            if args.root:
                if not dataFramePtCutSelContr.empty:
                    ...
                    WriteTree(dataFramePtCutSelContr, colsToKeep, outTreeName,
                            f'{outDirName}/{labelsContr[contr]}{outSuffix}_pT_{PtMin[iPt]:.0f}_{PtMax[iPt]:.0f}.root')
            else:
                if not dataFramePtCutSelContr.empty:
                    print(f'Saving {labelsContr[contr]} parquet')
                    dataFramePtCutSelContr[colsToKeep].to_parquet(
                        f'{outDirName}/{labelsContr[contr]}{outSuffix}_pT_{PtMin[iPt]:.0f}_{PtMax[iPt]:.0f}.parquet.gzip',
                        compression='gzip')
    else:
        if args.root:
            print('Saving data tree')
            WriteTree(dataFramePtCutSel, colsToKeep, outTreeName,
                    f'{outDirName}/Data{outSuffix}_pT_{PtMin[iPt]:.0f}_{PtMax[iPt]:.0f}.root')
        else:
            print('Saving data parquet')
            dataFramePtCutSel[colsToKeep].to_parquet(\
                f'{outDirName}/Data{outSuffix}_pT_{PtMin[iPt]:.0f}_{PtMax[iPt]:.0f}.parquet.gzip', compression='gzip')
>>>>>>> Added support for different pt ranges in a single config file
