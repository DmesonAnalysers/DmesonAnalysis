...
for iPt in range(hRawYields[0].GetNbinsX()):
    ...
    if cutSetCfg['linearplot']['enable']:
        ...
        legendLinearPlot.append(TLegend(0.1, 0.1, 0.6, 0.3))
    ...
    if doRawYieldsSmearing:
        ...
        for iRawYeld, _ in enumerate(listRawYield):
            ...
            if cutSetCfg['minimisation']['correlated']:
                if iRawYeld == 0:
                    listRawYieldSmeared.append(gRandom.PoissonD(listRawYield[iRawYeld] + listBkg[iRawYeld]) - listBkg[iRawYeld])
                else:
                    listRawYieldSmeared.append(listRawYieldSmeared[iRawYeld-1] + listDeltaSmeared[iRawYeld] -listDeltaBkg[iRawYeld])
            else:
    ...
    for iCutSet, (rawY, effP, effF, rawYunc, effPunc, effFunc) in enumerate(zip(listRawYield, listEffPrompt, listEffFD,
        ...
        if cutSetCfg['linearplot']['enable']:
            ...
            if iCutSet != 0: 
                ...
                legendLinearPlot[iPt].AddEntry(fNfdNprompt[iPt][iCutSet],f'cutset{iCutSet+1}')
            else:
