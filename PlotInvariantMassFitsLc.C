...
int PlotInvariantMassFitsLc() {
  ...
  TString infilename="rawYields/basic/ry_basic_PromptEn.root";
  TString canvasname="cMass0";
  ...
  for(int iPt=0; iPt<4; iPt++) {
    ...
    double rawyield = fTot[iPt]->GetParameter(1)/hMass[iPt]->GetBinWidth(1);
    double rawyielderr = fTot[iPt]->GetParError(1)/hMass[iPt]->GetBinWidth(1);
  ...
  double ymin[4] = {16e4, 29e3, 3.5e3, 0.5e3};
  double ymax[4] = {22e4, 38e3, 6e3, 1.1e3};
  //double ymin[4] = {80e3, 14e3, 2300, 600};
  ...
  TCanvas *coutput = new TCanvas("coutput","",1100,1100);
  coutput->Divide(2,2);
  TH1F* hFrame[4];
  for(int iPt=0; iPt<4; iPt++) {
